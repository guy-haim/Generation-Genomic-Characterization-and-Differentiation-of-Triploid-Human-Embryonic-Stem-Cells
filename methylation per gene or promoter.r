...
setwd("C:/Users/owner/Desktop/bisulfite sequencing")
...
B123=read.delim("b123_es_galaxy.bedGraph", header = F)[-1,]
...
colnames(B123)=c("chr","start","end","meth_percentage","num_meth","num_un_meth")
...
B123$location=paste(B123[,1],B123[,2],sep="-")
...
B123=B123[(B123$num_meth+B123$num_un_meth)>=10,]
SwapS4=SwapS4[(SwapS4$num_meth+SwapS4$num_un_meth)>=10,]

# gtf=read.delim("C:/Users/owner/Desktop/bisulfite sequencing/gencode.v38.primary_assembly.annotation.gtf", skip=5, header = F)
...
# gtf$Ensembl_ID=substring(gtf$V9, regexpr("gene_id", gtf$V9)+8,regexpr("; g", gtf$V9)-1)
...
if (!file.exists("methylation per promoter of gene.csv")){
  ...
  for (i in 1:23){
    ...
    sub_chr_B123=B123[B123$chr==chr, ]
    ...
    for (j in 1:nrow(sub_promoter_location)){
      ...
      promoter_location$B123[which(promoter_location$chr==chr)[1]+j-1]=mean(sub_chr_B123$meth_percentage[sub_chr_B123$start%in%start:end])
...
if (!file.exists("methylation per gene.csv")){
  ...
  for (i in 1:23){
    ...
    for (j in 1:nrow(sub_location)){
      ...
      location$B123[which(location$chr==chr)[1]+j-1]=mean(sub_chr_B123$meth_percentage[sub_chr_B123$start%in%start:end])
...
for (i in 1:23){
  ...
  lines(c(sum+centromere_pos[i],sum+centromere_pos[i]),c(-12.5,-7.2),col="grey13",lwd=2)
  sum=sum+chr_size[i]
}
